...
class CharStream(private val data: String) {
    private var row: Int = 1
    private var column: Int = 0
    private var position: Int = 0

    fun peek(): Char? {
        return if (position >= data.length) {
            null
        } else {
            data[position]
        }
...
fun main() {
    val stream = CharStream(FileReader("./src/main/kotlin/chapter02/simple.z").readText())

    while (!stream.eof()) {
        println("current:" + stream.next())
